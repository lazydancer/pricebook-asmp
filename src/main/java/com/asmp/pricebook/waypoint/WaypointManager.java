...
public final class WaypointManager {
    ...
    private static void registerWorldProjection() {
        WorldRenderEvents.AFTER_ENTITIES.register(ctx -> {
            ...
            Vec3d worldPos = new Vec3d(waypoint.pos.getX() + 0.5, waypoint.pos.getY() + 1.2, waypoint.pos.getZ() + 0.5);
    ...
    private static void registerHudElement() {
        HudElement element = (DrawContext context, RenderTickCounter tickCounter) -> {
            ...
            int y = screenY - textHeight - 6;
            x = Math.max(2, Math.min(x, sw - textWidth - 2));
            y = Math.max(2, Math.min(y, sh - textHeight - 2));
            int bg = 0x80000000;
            context.fill(x - padding, y - padding, x + textWidth + padding, y + textHeight + padding, bg);
            context.drawText(renderer, line, x, y, 0xFFFFFFFF, true);
        };
    ...
    private static void registerAutoClearNearGoal() {
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (client.player.getPos().distanceTo(Vec3d.ofCenter(waypoint.pos)) < 10) {
                clear();
            }
        });
    }
